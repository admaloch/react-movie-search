...
const getMovieReviews = async (req, res) => {
  ...
  const reviews = await ReviewModel.find({ imdbId: movieId }).populate(
    "user",
    "username",
  );
  if (!reviews) {
...
const createReview = async (req, res) => {
  ...
  if (alreadyReviewed) {
    return res
      .status(409)
      .json({ message: "You have already submitted a review for this movie" });
  }
  ...
  res
    .status(201)
    .json({
      message: `New review for imdb id # ${imdbId} created by ${user}`,
      review,
    });
};
...
const updateReview = async (req, res) => {
  ...
  if (!body || !rating || !id) {
    return res
      .status(400)
      .json({ message: "Review ID and either body or rating are required" });
  }
