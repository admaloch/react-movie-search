...
const MainSlider = ({
  ...
  const updateProgBarHandler = (progBarArr: ProgBar[]) =>
    dispatch({ type: "updateProgBar", progBarArr: progBarArr });
  const increaseIndexHandler = () => dispatch({ type: "increment" });
  const decreaseIndexHandler = () => dispatch({ type: "decrement" });
  const changeIndexHandler = (newIndex: number) =>
    dispatch({ type: "changeIndex", newIndex: newIndex });
